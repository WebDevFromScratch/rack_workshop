...
describe Rack::RateLimiterPa do
  ...
  describe 'Custom store' do
    ...
    let(:store) { double(:store, get: { limit_remaining: 10, limit_reset_at: reset_at }, set: nil) }
    ...
    it 'correctly updates the id to the store' do
      expect(store).to receive(:set).with("something", { limit_remaining: 9, limit_reset_at: reset_at })
      get '/'
    end
  end
end
